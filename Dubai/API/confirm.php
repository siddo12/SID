...
<html lang="en">
 <head>
  ...
  <title>API</title>
  <h1>Confirm</h1>
  <p>Confirm API call allows you to confirm a pending order. Only confirmed orders are settled into the
merchant’s account. An order older than 7 days is automatically cancelled. Once an order has been
auto-cancelled by the system, it cannot be confirmed. You can optionally confirm only part of an order.
Remember, you can do so only once. Remaining funds will be refunded to the credit card or debit card or
net banking account that was originally charged.</p>
 </head>
 <body>
 ...
 <tr><td>Capture Amount :</td> <td><input type ="text" name ="amount"></td></tr>
...
'order_List' =>
    ...
    $final_data = "request_type=JSON&response_type=JSON&access_code=".$access_code."&command=confirmOrder&version=1.1&enc_request=" . $encrypted_data;
