...
<html lang="en">
 <head>
  ...
  <title>API</title>
  <h1>Order Look Up</h1>
  <p>The Lookup API call can be used to extract transaction details for a certain set of transactions. The
functionality works similar to a search feature.</p>
 </head>
 <body>
  <form action="<?php $_SERVER["PHP_SELF"]; ?>" name="form1" method="post"> 
 <table>
 
 <tr><td>Order Date From :</td> <td><input type ="text" name ="from_date"></td> <br> <td>To :</td> <td><input type ="text" name ="from_date"></td></tr>
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $final_data = "request_type=JSON&response_type=JSON&access_code=".$access_code."&command=orderLookup&version=1.1&enc_request=" . $encrypted_data;
